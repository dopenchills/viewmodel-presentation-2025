...
フロントエンドは人間が使いやすいように作られている

バックエンドはアプリケーションが使いやすいように作られている

しかし前述の通り、核となるロジックさえ切り出せば、内部的な構造は似ている

インターフェースが違うだけで、バックエンドを人間が使うこともできるし、フロントエンドをアプリケーションが使うこともできる
...
また、これを守ることで、核となるロジックには別の各種手法を導入可能。フロントエンドかバックエンドかは関係ない。
...
例えばフロントエンドでは通信環境が悪いことが多く、キャッシュが必要なこともある。そのような要件は外部システムとのやりとりにまつわる話なので、周辺ロジックでの実装になる。
...
永続化のロジックの抽象に依存することで、バックエンド非依存にできる。
